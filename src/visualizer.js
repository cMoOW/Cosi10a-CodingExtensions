    ...
    const tabChangeListener = vscode.window.onDidChangeActiveTextEditor(editor => {
        ...
        if (editor && editor.document.languageId === 'python') {
            if (!associatedDocument || editor.document.uri.toString() !== associatedDocument.uri.toString()) {
                ...
                // Preserve currentInput across tab switches
                // Optionally preserve seed; keeping reset to 42 for determinism unless randomness controls are used
                currentSeed = "42";
...
function createOrShowPanel() {
    ...
    visualizerPanel.webview.html = getVisualizerHtml(sourceCode, "[]", currentInput, null, showInputBox, hasRandomness);
...
function onDocumentChange(event) {
    ...
    // Clear decorations immediately on edit
    const editor = vscode.window.activeTextEditor;
    ...
    return `
        ...
        <body>
            ...
            <script>
                ...
                // Syncing only after user interacts with the panel
