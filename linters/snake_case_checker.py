...
class SnakeCaseChecker(BaseChecker) :
    ...
    msgs = { # Message for students
        ...
        "C9001": ( # Message ID, C=convention, list ends at C0414
            "Function name '%s' should be in snake_case.",
            "invalid-function-name-snake-case",
            "Used when a function name is not snake_case.",
        ),
    }
    ...
    def visit_functiondef(self, node):
        # Check function name
        func_name = node.name
        if not SNAKE_CASE_REGEX.match(func_name) and not SCREAMING_SNAKE_CASE_REGEX.match(func_name) :
            self.add_message("invalid-function-name-snake-case", node=node, args=(func_name,))

        # Check parameter names
        for arg in node.args.args:  # Regular parameters
            if not SNAKE_CASE_REGEX.match(arg.name):
                self.add_message("invalid-variable-name-snake-case", node=arg, args=(arg.arg,))

        if node.args.vararg:  # *args
            if not SNAKE_CASE_REGEX.match(node.args.vararg.name):
        ...
        if node.args.kwarg:  # **kwargs
            if not SNAKE_CASE_REGEX.match(node.args.kwarg.name):
